...
public class EncryptorClientClass {
    public static String main(String password) {
        Scanner keyboard = new Scanner(System.in);
//
        EncryptorClass newClassObj = new EncryptorClass();
        ...
        to organize it
         */
        String[] brokenString = EncryptorClass.stringBreaker(password);
        ...
        for (int i = 0; i <= brokenString.length - 1; i++) {
            ...
            charLocation[i] = EncryptorClass.findIndex(alphabetArray, alphacharLocation);
        }
        //The above line is checking at what point letter in brokenArray[i] occurs in alphabetArray
        //the above code is for debugging
        int maxCharLocation = EncryptorClass.max(charLocation);
//            System.out.print("What number are you making the code by? (-" +((((alphabetArray.length) - (maxCharLocation)) + (brokenString.length - 3)) + 1 +alphabetArray.length-brokenString.length)+") to ("+ ((((alphabetArray.length) - (maxCharLocation)) + (brokenString.length - 3)) + 1 +alphabetArray.length-brokenString.length) + ")");
        ...
        int[] shiftArray = EncryptorClass.shiftString(brokenString, alphabetArray,charLocation);
        String str = EncryptorClass.alphaDecode(brokenString,shiftArray,alphabetArray);
